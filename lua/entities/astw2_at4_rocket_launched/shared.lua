            ...
            if self.Owner:IsValid() and self.Owner:GetActiveWeapon().Tracking and self.Owner:GetActiveWeapon().ThirdPersonWeapon and self.Owner:GetActiveWeapon():GetNWBool("insights") or self.Owner:IsNPC() then
                ...
                if target then
                    if neededangle:Forward():Dot( self:GetAngles():Forward() ) >= self.TrackingAngle then
                        ...
                        phys:ApplyForceCenter( self:GetAngles():Forward() * 2000 )
                    end
                end
            end
...
function ENT:Detonate()
    if SERVER then
        ...
        util.BlastDamage(self, attacker, self:GetPos(), 128, 250)
    ...
    if CLIENT then
        ...
        if self.Armed then
            cam.Start3D() -- Start the 3D function so we can draw onto the screen.
                ...
                render.DrawSprite( self:GetPos(), math.random(200, 300), math.random(200, 300), Color(255, 200, 150) ) -- Draw the sprite in the middle of the map, at 16x16 in it's original colour with full alpha.
            cam.End3D()
        end
    end
end