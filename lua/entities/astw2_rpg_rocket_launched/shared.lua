...
function ENT:Detonate()
    if SERVER then
        ...
        util.BlastDamage(self, attacker, self:GetPos(), 128, 350)
    ...
    if CLIENT then
        ...
        if self.Armed then
            cam.Start3D() -- Start the 3D function so we can draw onto the screen.
                ...
                render.DrawSprite( self:GetPos(), math.random(200, 300), math.random(200, 300), Color(255, 175, 150) ) -- Draw the sprite in the middle of the map, at 16x16 in it's original colour with full alpha.
            cam.End3D()
        end
    end
end