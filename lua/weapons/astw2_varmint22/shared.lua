
SWEP.Spawnable = true
SWEP.AdminOnly = false
SWEP.Base = "astw2_base"

SWEP.PrintName = "Varmint LR .22"
SWEP.Category = "ASTW2 - Carbines"
SWEP.Slot = 3
SWEP.Author = "Arctic"
SWEP.Contact = "/id/ArcticWinterZzZ/"
SWEP.Purpose = "A version of the Varmint Rifle chambered in the lighter .22 LR round."
SWEP.Instructions = "ATTACK2 to aim, ATTACK1 to fire. Uses .22 ammo."

SWEP.ViewModel = nil
SWEP.WorldModel = "models/weapons/w_rif_varmint22.mdl"

SWEP.Primary.Damage = 6
SWEP.Primary.Delay = 60 / 100
SWEP.Primary.Acc = 1 / 225
SWEP.Primary.Recoil = 175
SWEP.Primary.RecoilAcc = 100
SWEP.Primary.Num = 1
SWEP.Primary.Automatic = false
SWEP.Primary.Ammo = ".22"
SWEP.Primary.ClipSize = 20

SWEP.Projectile = nil
SWEP.ProjectileForce = 2500

SWEP.Sound = "weapons/fiveseven/fiveseven-1.wav"
SWEP.Sound_Vol = 100
SWEP.Sound_Pitch = 175
SWEP.Sound_Magout = "weapons/g3sg1/g3sg1_clipout.wav"
SWEP.Sound_Magin = "weapons/famas/famas_clipin.wav"
SWEP.Sound_Boltpull = "weapons/k98_singleshotreload.wav"
SWEP.Sound_BoltActionDelay = 0.1
SWEP.Sound_BoltAction = "weapons/scout/scout_bolt.wav"
SWEP.Sound_BoltActionPitch = 145

SWEP.Secondary.Ammo = nil

SWEP.Effect_ShellEject = "astw2_case_22lr"

SWEP.MagDrop = ""

SWEP.ReloadTime = 2
SWEP.CannotChamber = false

SWEP.MuzzlePosOverride = Vector(2, 30, -8)

SWEP.UniqueFiremode = "BOLT"

SWEP.HoldType_Lowered = "passive"
SWEP.HoldType_Aim = "ar2"
SWEP.Anim_Reload = ACT_HL2MP_GESTURE_RELOAD_SMG1
SWEP.Anim_Shoot = ACT_HL2MP_GESTURE_RANGE_ATTACK_RPG